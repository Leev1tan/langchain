...
def compare_sql_syntax(generated_sql: str, gold_sql: str) -> Tuple[bool, float, str]:
    """
    Compare generated SQL with gold standard SQL to determine similarity
    ...
    Returns:
        Tuple of (exact_match, similarity_score, comparison_notes)
    """
    ...
    def extract_components(sql):
        ...
        if where_match:
            ...
            components['where'] = [cond.strip() for cond in re.split(r'\s+and\s+|\s+or\s+', where_clause, flags=re.IGNORECASE) if cond.strip()]
        ...
        if having_match:
            ...
            components['having'] = [cond.strip() for cond in re.split(r'\s+and\s+|\s+or\s+', having_clause, flags=re.IGNORECASE) if cond.strip()]
    ...
    gen_components = extract_components(normalized_generated)
    gold_components = extract_components(normalized_gold)
    
    # Calculate similarity scores for each component
    similarity_scores = {}
    
    # Helper function to calculate Jaccard similarity between two lists
    ...
    for component in ['select', 'from', 'where', 'group_by', 'order_by', 'having']:
        similarity_scores[component] = jaccard_similarity(gen_components[component], gold_components[component])
    
    # LIMIT is a special case
    if gen_components['limit'] == gold_components['limit']:
        similarity_scores['limit'] = 1.0
    elif gen_components['limit'] is None or gold_components['limit'] is None:
        similarity_scores['limit'] = 0.0
    else:
        # Calculate similarity based on how close the limits are
        max_limit = max(gen_components['limit'] or 0, gold_components['limit'] or 0)
        min_limit = min(gen_components['limit'] or 0, gold_components['limit'] or 0)
        if max_limit > 0:
            similarity_scores['limit'] = min_limit / max_limit
        else:
            similarity_scores['limit'] = 0.0
    
    # Calculate overall similarity with weighted components
    weights = {
        'select': 0.3,  # SELECT is very important
        'from': 0.2,    # FROM is important
        'where': 0.25,  # WHERE conditions are important
        'group_by': 0.1,
        'order_by': 0.05,
        'having': 0.05,
        'limit': 0.05
    ...
    overall_similarity = sum(similarity_scores[comp] * weights[comp] for comp in weights)
    
    # Generate comparison notes
    notes = []
    for component in ['select', 'from', 'where', 'group_by', 'order_by', 'having', 'limit']:
        if similarity_scores[component] < 1.0:
            if component == 'limit':
                notes.append(f"{component.upper()}: {gen_components[component]} vs {gold_components[component]}")
            else:
                gen_items = gen_components[component]
                gold_items = gold_components[component]
                
                if not gen_items and gold_items:
                    notes.append(f"Missing {component.upper()} clause")
                elif gen_items and not gold_items:
                    notes.append(f"Extra {component.upper()} clause")
                else:
                    missing = set(gold_items) - set(gen_items)
                    extra = set(gen_items) - set(gold_items)
                    
                    if missing:
                        notes.append(f"Missing in {component.upper()}: {', '.join(missing)}")
                    if extra:
                        notes.append(f"Extra in {component.upper()}: {', '.join(extra)}")
    
    comparison_notes = "; ".join(notes) if notes else "Queries are structurally similar but not identical"
    
    # Determine if it's a good match (80% similarity threshold)
    is_good_match = overall_similarity >= 0.8
    
    return is_good_match, overall_similarity, comparison_notes
...
def evaluate_mac_sql(benchmark_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    results = {
        ...
        "sql_syntax_match": 0,   # Queries with matching SQL syntax
        "execution_errors": 0,
        "execution_times": [],
        "detailed_results": []
    }
    ...
    for i, item in enumerate(tqdm(benchmark_data, desc="Evaluating queries")):
        ...
        sql_match = False
        sql_similarity = 0.0
        sql_comparison_notes = ""
        
        if sql and gold_sql:
            sql_match, sql_similarity, sql_comparison_notes = compare_sql_syntax(sql, gold_sql)
        ...
        detail = {
            ...
            "sql_comparison_notes": sql_comparison_notes,
        ...
        if sql_match:
            results["sql_syntax_match"] += 1
            print(f"SQL syntax matches gold standard: True (similarity: {sql_similarity:.2f})")
        else:
            print(f"SQL syntax matches gold standard: False (similarity: {sql_similarity:.2f})")
            if sql_comparison_notes:
                print(f"SQL comparison notes: {sql_comparison_notes}")
    ...
    results["sql_syntax_accuracy"] = results["sql_syntax_match"] / results["total"] if results["total"] > 0 else 0
    ...
    for detail in results["detailed_results"]:
        ...
        if detail["sql_match"]:
            results["results_by_db"][db_id]["sql_syntax_match"] += 1
    ...
    for db_id, db_result in results["results_by_db"].items():
        ...
        db_result["sql_syntax_accuracy"] = db_result["sql_syntax_match"] / db_result["total"] if db_result["total"] > 0 else 0
    ...
    for detail in results["detailed_results"]:
        ...
        if detail["sql_match"]:
            results["results_by_difficulty"][difficulty]["sql_syntax_match"] += 1
    ...
    for difficulty, diff_result in results["results_by_difficulty"].items():
        ...
        diff_result["sql_syntax_accuracy"] = diff_result["sql_syntax_match"] / diff_result["total"] if diff_result["total"] > 0 else 0
    ...
    for detail in results["detailed_results"]:
        ...
        if detail["sql_match"]:
            results["results_by_hardness"][hardness]["sql_syntax_match"] += 1
    ...
    for hardness, hard_result in results["results_by_hardness"].items():
        ...
        hard_result["sql_syntax_accuracy"] = hard_result["sql_syntax_match"] / hard_result["total"] if hard_result["total"] > 0 else 0
...
def compare_query_results(generated_df: pd.DataFrame, gold_df: pd.DataFrame) -> Tuple[bool, str]:
    ...
    # Check row count match - this is a strict requirement for most queries
    ...
    if gen_cols != gold_cols:
        # Check if column count matches but names differ - this might be an alias issue
        if len(gen_cols) == len(gold_cols):
            # If column counts match but names differ, try to compare by position
            return compare_by_column_position(generated_df, gold_df)
        else:
    ...
    # We'll try to compare the data regardless of row order by converting to sets of tuples
    try:
        ...
        if gen_tuples == gold_tuples:
            ...
            # If they don't match exactly, check what percentage of rows match
            intersection = gen_tuples.intersection(gold_tuples)
            match_percentage = len(intersection) / len(gold_tuples) * 100
            
            if match_percentage >= 95:  # If at least 95% of rows match
                return True, f"Results match approximately ({match_percentage:.1f}% overlap)"
            else:
                return False, f"Results don't match as sets (only {match_percentage:.1f}% overlap)"
    except Exception as e:
        # If tuple conversion fails (e.g., due to unhashable types)
        # Try a row-by-row comparison with some flexibility
        return compare_rows_with_tolerance(generated_df, gold_df)

def compare_by_column_position(generated_df: pd.DataFrame, gold_df: pd.DataFrame) -> Tuple[bool, str]:
    """Compare DataFrames by column position rather than column names"""
    # Reset column names to positional indices for both dataframes
    gen_df_reset = generated_df.copy()
    gold_df_reset = gold_df.copy()
    
    gen_df_reset.columns = range(len(gen_df_reset.columns))
    gold_df_reset.columns = range(len(gold_df_reset.columns))
    
    # Now try exact comparison
    if gen_df_reset.equals(gold_df_reset):
        return True, "DataFrames match exactly when comparing by column position"
    
    # Try set comparison
    try:
        gen_tuples = set(map(tuple, gen_df_reset.values))
        gold_tuples = set(map(tuple, gold_df_reset.values))
        
        if gen_tuples == gold_tuples:
            return True, "Results match as sets by column position (ignoring row order)"
        else:
            # Calculate overlap percentage
            intersection = gen_tuples.intersection(gold_tuples)
            match_percentage = len(intersection) / len(gold_tuples) * 100
            
            if match_percentage >= 90:  # If at least 90% of rows match
                return True, f"Results match approximately by column position ({match_percentage:.1f}% overlap)"
            else:
                return False, f"Results don't match as sets by column position (only {match_percentage:.1f}% overlap)"
    except:
        return False, "Unable to compare results as sets by column position"

def compare_rows_with_tolerance(generated_df: pd.DataFrame, gold_df: pd.DataFrame) -> Tuple[bool, str]:
    """Compare rows with numeric tolerance and string flexibility"""
    # We'll count how many rows have good matches
    total_rows = len(gold_df)
    matching_rows = 0
    
    # For each row in gold standard
    for g_idx, gold_row in gold_df.iterrows():
        # Try to find a matching row in generated results
        for gen_idx, gen_row in generated_df.iterrows():
            match = True
            ...
            for col in gold_df.columns:
                ...
                # Find matching column in generated df
                gen_col = col
                if col not in generated_df.columns:
                    # Try to find a column with similar name
                    for gcol in generated_df.columns:
                        if col.lower() in gcol.lower() or gcol.lower() in col.lower():
                            gen_col = gcol
                            break
                    else:
                ...
                gen_val = gen_row[gen_col]
                
                # Handle numeric comparison with tolerance
                if pd.api.types.is_numeric_dtype(type(gold_val)) and pd.api.types.is_numeric_dtype(type(gen_val)):
                    try:
                        ...
                        # Use relative tolerance for large numbers
                        if abs(gold_num) > 1.0:
                            rel_diff = abs(gold_num - gen_num) / abs(gold_num)
                            if rel_diff > 0.01:  # 1% tolerance
                                match = False
                                break
                        # Use absolute tolerance for small numbers
                        elif abs(gold_num - gen_num) > 0.001:
                            match = False
                            break
                    except (ValueError, TypeError):
                        # If conversion to float fails, do exact comparison
                        if str(gold_val) != str(gen_val):
                            match = False
                            break
                # String comparison (case insensitive for text)
                elif isinstance(gold_val, str) and isinstance(gen_val, str):
                    if gold_val.lower() != gen_val.lower():
                        match = False
                        break
                # Other types - exact comparison
                elif gold_val != gen_val:
                    match = False
                    break
            
            # If all columns matched for this row
            if match:
                matching_rows += 1
                break  # Move to next gold row
    
    # Calculate match percentage
    match_percentage = (matching_rows / total_rows) * 100 if total_rows > 0 else 0
    
    if match_percentage >= 95:
        return True, f"Row-by-row comparison shows {match_percentage:.1f}% match"
    else:
        return False, f"Row-by-row comparison shows only {match_percentage:.1f}% match"
...
def visualize_results(results: Dict[str, Any]):
    ...
    db_sql_accuracy = [results["results_by_db"][db]["sql_syntax_accuracy"] * 100 for db in db_names]
    
    x = np.arange(len(db_names))
    width = 0.25  # Narrower bars to fit three categories
    
    axes[0, 0].bar(x - width, db_exec_accuracy, width, label='Execution')
    axes[0, 0].bar(x, db_semantic_accuracy, width, label='Semantic')
    axes[0, 0].bar(x + width, db_sql_accuracy, width, label='SQL Syntax')
    ...
    if "results_by_difficulty" in results:
        ...
        diff_sql_accuracy = [results["results_by_difficulty"][diff]["sql_syntax_accuracy"] * 100 for diff in diff_names]
        
        x = np.arange(len(diff_names))
        
        axes[0, 1].bar(x - width, diff_exec_accuracy, width, label='Execution')
        axes[0, 1].bar(x, diff_semantic_accuracy, width, label='Semantic')
        axes[0, 1].bar(x + width, diff_sql_accuracy, width, label='SQL Syntax')
    ...
    if "results_by_hardness" in results:
        ...
        hard_sql_accuracy = [results["results_by_hardness"][hard]["sql_syntax_accuracy"] * 100 for hard in hard_names]
        
        x = np.arange(len(hard_names))
        
        axes[1, 0].bar(x - width, hard_exec_accuracy, width, label='Execution')
        axes[1, 0].bar(x, hard_semantic_accuracy, width, label='Semantic')
        axes[1, 0].bar(x + width, hard_sql_accuracy, width, label='SQL Syntax')
    ...
    labels = ['Execution', 'Semantic', 'SQL Syntax']
    overall_accuracy = [
        results["execution_accuracy"] * 100, 
        results["semantic_accuracy"] * 100,
        results["sql_syntax_accuracy"] * 100
    ]
...
def print_results_summary(results: Dict[str, Any]):
    ...
    print(f"SQL Syntax Accuracy: {results['sql_syntax_accuracy'] * 100:.2f}%")
    ...
    print(f"SQL syntax matches: {results['sql_syntax_match']} / {results['total']}")
    
    print("\nResults by Database:")
    for db, db_result in results["results_by_db"].items():
        print(f"  - {db}: Exec={db_result['execution_accuracy'] * 100:.2f}%, Semantic={db_result['semantic_accuracy'] * 100:.2f}%, SQL={db_result['sql_syntax_accuracy'] * 100:.2f}% ({db_result['semantic_correct']}/{db_result['total']})")
    
    if "results_by_difficulty" in results:
        print("\nResults by Complexity:")
        for diff, diff_result in results["results_by_difficulty"].items():
            print(f"  - {diff}: Exec={diff_result['execution_accuracy'] * 100:.2f}%, Semantic={diff_result['semantic_accuracy'] * 100:.2f}%, SQL={diff_result['sql_syntax_accuracy'] * 100:.2f}% ({diff_result['semantic_correct']}/{diff_result['total']})")
    
    if "results_by_hardness" in results:
        print("\nResults by Difficulty:")
        for hard, hard_result in results["results_by_hardness"].items():
            print(f"  - {hard}: Exec={hard_result['execution_accuracy'] * 100:.2f}%, Semantic={hard_result['semantic_accuracy'] * 100:.2f}%, SQL={hard_result['sql_syntax_accuracy'] * 100:.2f}% ({hard_result['semantic_correct']}/{hard_result['total']})")
